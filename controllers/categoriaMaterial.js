...
// modelos
var CategoriaMaterial = require('../models/categoriaMaterial');

// acciones
function pruebas(req, res) {
    res.status(200).send({
        message: 'Probando el controlado de categoriaMaterial y la accion de pruebas',
...
function getAll(req, res) {
    CategoriaMaterial.find({"borrado":0}).exec((err, categorias)=> {
...
function getById(req, res) {
    ...
    CategoriaMaterial.findById(categoriaId).exec((err, categoria)=> {
        if (err) {
            res.status(500).send({message: 'error en la peticion...'});
        } else {
            if (!categoria) {
...
function update(req, res) {
    ...
    CategoriaMaterial.findByIdAndUpdate(categoriaId, update, {new: true}, (err, categoriaUpdated)=> {
        if (err) {
            res.status(500).send({message: 'error en la peticion...'});
        } else {
            if (!categoriaUpdated) {
